...
@SpringBootTest
@Transactional
class CityCoordinateMapperTest @Autowired constructor(
    private val cityCoordinateMapper: CityCoordinateMapper
) {

    @Test
    @DisplayName("서울의 좌표를 정확히 반환해야 한다")
    fun getCoordinate_ReturnsCorrectCoordinateForSeoul() {
        val coordinate = cityCoordinateMapper.getCoordinate("seoul")
    ...
    fun getCoordinate_ReturnsCorrectCoordinateIgnoreCase() {
        val coordinate = cityCoordinateMapper.getCoordinate("SEOUL")
        assertThat(coordinate).isNotNull
        assertThat(coordinate.latitude).isEqualTo(37.5665)
        assertThat(coordinate.longitude).isEqualTo(126.9780)
    ...
    fun getCoordinate_ThrowsExceptionForUnsupportedCity() {
    ...
    fun getCoordinate_ThrowsExceptionForNullCity() {
